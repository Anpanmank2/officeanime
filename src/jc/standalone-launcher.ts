...
async function main(): Promise<void> {
  ...
  const workflow = createRequestWorkflow({
    file: path.join(os.homedir(), '.pixel-agents', 'requests.json'),
    config: jcConfig as JCConfig,
    workspaceRoot: step2WorkspaceRoot,
    eventsFile: ownerEventsFile,
    broadcast: (message) => server.broadcast(message),
    allowDrafts: () => liveSpawnEnabled,
    history: { writeEntry: (...args) => taskWatcher?.getHistoryWriter().writeEntry(...args) },
  });
  ...
  const server = await startBrowserServer(extensionPath, port, (data, respond) => {
    ...
    if (msg.type === 'webviewReady') {
      ...
      respond({ type: 'jcWorkSnapshot', requests: workflow.snapshot() });
      return;
    }

    if (workflow.handle(data, respond)) return;
    ...
    if (msg.type === 'jcApprovalAnswer') {
      ...
      try {
        const prior = validateApprovalAnswer(ownerEventsFile, d.request_id, d.answer, companyId);
        if (prior) {
          respond({ type: 'jcOfficeEvent', event: prior });
          return;
        }
        ...
        server.broadcast({ type: 'jcOfficeEvent', event: approvalResolved });
      } catch (e) {
        respond({ type: 'jcApprovalAnswerError', requestId: d.request_id, error: String(e) });
  ...
  const workflowTimer = setInterval(() => workflow.expire(), 1000);
