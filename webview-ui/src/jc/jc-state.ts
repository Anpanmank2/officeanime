...
/** Desk positions — mirrors the extension-side desk-registry */
const DESK_POSITIONS: Record<
  string,
  { col: number; row: number; facingDir: number; nameplate: string; nameplateEn: string }
> = {
  // Engineering
  'dev-desk-01': { col: 15, row: 7, facingDir: 3, nameplate: '田中 健太', nameplateEn: 'K.Tanaka' },
  'dev-desk-02': { col: 18, row: 7, facingDir: 3, nameplate: '佐藤 涼', nameplateEn: 'R.Sato' },
  'dev-desk-03': {
    col: 15,
    row: 9,
    facingDir: 3,
  ...
  'dev-desk-05': { col: 15, row: 11, facingDir: 3, nameplate: '藤井 蓮', nameplateEn: 'R.Fujii' },
  'dev-desk-06': {
    col: 18,
    row: 11,
    facingDir: 3,
  ...
  // Marketing
  'mkt-desk-01': { col: 3, row: 13, facingDir: 3, nameplate: '黒田 涼', nameplateEn: 'R.Kuroda' },
  'mkt-desk-02': { col: 6, row: 13, facingDir: 3, nameplate: '伊藤 美咲', nameplateEn: 'M.Ito' },
  'mkt-desk-03': { col: 9, row: 13, facingDir: 3, nameplate: '渡辺 蓮', nameplateEn: 'R.Watanabe' },
  'mkt-desk-04': {
    col: 3,
    row: 15,
    facingDir: 3,
  ...
  'mkt-desk-05': { col: 6, row: 15, facingDir: 3, nameplate: '加藤 翼', nameplateEn: 'T.Kato' },
  'mkt-desk-06': {
    col: 9,
    row: 15,
    facingDir: 3,
  ...
  'mkt-desk-08': { col: 6, row: 17, facingDir: 3, nameplate: '井上 花', nameplateEn: 'H.Inoue' },
  'mkt-desk-09': { col: 9, row: 17, facingDir: 3, nameplate: '木村 翔', nameplateEn: 'S.Kimura' },
  'mkt-desk-10': { col: 11, row: 15, facingDir: 3, nameplate: '林 優衣', nameplateEn: 'Y.Hayashi' },
  'mkt-desk-11': {
    col: 11,
    row: 17,
    facingDir: 3,
  ...
  // Research
  'res-desk-01': { col: 16, row: 13, facingDir: 3, nameplate: 'Owner', nameplateEn: 'Owner' },
  'res-desk-02': {
    col: 19,
    row: 13,
    facingDir: 3,
  ...
  'res-desk-03': { col: 16, row: 15, facingDir: 3, nameplate: '森 颯太', nameplateEn: 'S.Mori' },
  'res-desk-04': {
    col: 19,
    row: 15,
    facingDir: 3,
  ...
  'res-desk-06': { col: 19, row: 17, facingDir: 3, nameplate: '空席', nameplateEn: 'Vacant' },
};
...
export function jcLoadConfig(config: JCConfigData): void {
  jcConfig = config;
  // Initialize runtime state for all members
...
export function jcGetNameplates(): NameplateInfo[] {
  ...
  for (const [deskId, pos] of Object.entries(DESK_POSITIONS)) {
    // Find which member this desk belongs to
    let isPresent = false;
    if (jcConfig) {
    ...
    nameplates.push({
      text: pos.nameplateEn, // Use English short name for pixel rendering
      col: pos.col,
      row: pos.row,
      isPresent,
...
export function jcGetPresentMemberIds(): Set<string> {
  ...
  for (const [memberId, runtime] of memberRuntimes) {
